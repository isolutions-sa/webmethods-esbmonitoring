...
%ifvar mode equals('edit')%
   ...
   <BODY onLoad="setNavigation('propfile_settings-extended.dsp?fileName=%value fileName%', '');">
%else%
  ...
  <TABLE WIDTH="100%">
    <TR>
      <TD class="menusection-Settings" colspan="4">
        Property File Parameters</TD>
    </TR>
    <TR>

%ifvar action equals('change')%
  %rename propertyFile Todd -copy%
  %invoke ESBMonitoring.properties.services.packages:updatePkgProperties%
    %ifvar message%
...
%endif%
    ...
    <TR>
      <TD colspan=2>
        <UL>
          %ifvar mode equals('edit')%
          <LI><A HREF="propfile_settings-extended.dsp?fileName=%value fileName%">Return to Property File Settings</A></LI>
          %else%
          %ifvar pkg matches('Wm*')%
					%else%
          <LI><A HREF="propfile_settings-extended.dsp?mode=edit&fileName=%value fileName%">Edit Property File Settings</A></LI>
          %endif%%endif%
        </UL>
    </TR>
    <TR>
      ...
      <TD>
          <FORM action="propfile_settings-extended.dsp?fileName=%value fileName%" method="POST">
          <INPUT type="hidden" name="action" value="change">
		  <INPUT type="hidden" name="Package" value=%value pkg%>
          <INPUT type="hidden" name="propertyFile" value=%value fileName%>

        <TABLE class="tableForm" %ifvar mode equals('edit')%width="100%"%endif% >

%invoke ESBMonitoring.properties.services.file:readFile%
